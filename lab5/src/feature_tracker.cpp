...
void FeatureTracker::trackFeatures(
  ...
  //     **** TODO: FILL IN HERE ***
    ...
    if (save_images){
      imwrite("img_1_keypoints" , img_keypoints_1 );
      imwrite("img_2_keypoints" , img_keypoints_2 );
    }

  if (show_images){

    imshow(" img 1 keypoints" , img_keypoints_1 );
    imshow(" img 2 keypoints" , img_keypoints_2 );
    waitKey(10); 
  }
  ...
  describeKeypoints(img_1 , &keypoints_1, &descriptors1 );
  describeKeypoints(img_2 , &keypoints_2, &descriptors2 );

  LOG(INFO) <<"The size of cv::Mat descriptor size is " << descriptors1.rows <<" " << descriptors1.cols;
  ...
  //
  //     **** TODO: FILL IN HERE ***
  matchDescriptors(descriptors1 , descriptors2 ,
                  &matches , 
                &good_matches);
  //
  // ~~~~ end solution
  ...
  for (const auto& m : good_matches) {
      match_kp_1_kp_2.first.push_back(keypoints_1[m.queryIdx]);
      match_kp_1_kp_2.second.push_back(keypoints_2[m.trainIdx]);
  }
  ...
  // ~~~~ begin solution
    ...
    cv::Mat img_inlier;
    // draw everything with red
    cv::drawMatches( img_1, keypoints_1, img_2, keypoints_2, good_matches,
                    img_inlier, Scalar(255,0,0), Scalar(255,0,0),
                    std::vector<char> (),  
                    DrawMatchesFlags::NOT_DRAW_SINGLE_POINTS );
    
    // overlay green lines for inliners
    cv::drawMatches( img_1, keypoints_1, img_2, keypoints_2, good_matches,
                    img_inlier, Scalar(0,255,0), Scalar(0,255,0),
                    char_inlier_mask,  // vector of size equal to matches - draw it or not
                    DrawMatchesFlags::NOT_DRAW_SINGLE_POINTS | DrawMatchesFlags::DRAW_OVER_OUTIMG  // dont draw keypoints that are not matched
                  );

    if (show_images){
    imshow("inline_features", img_inlier);
    waitKey(10); 
  }

    // std::vector<uchar> outlier_mask;
    // for (const auto &v : inlier_mask){
    //   outlier_mask.push_back(!v);
    // }
    // std::vector<char> char_outlier_mask{outlier_mask.begin(), outlier_mask.end()}
    ...
    // imshow("Inliners and outliners", img_inlier );
    // waitKey(10);
