...
class PoseEstimator : public rclcpp::Node {
 public:
  ...
  float threshold = 0.1;  // trial and error
  float maxIterations = 256;  // maximum n for 8 point algo and assuming w == 0.5
  ...
  void poseCallbackTesse(const nav_msgs::msg::Odometry::ConstSharedPtr& msg) {
    ...
    // body frame to the drone's camera to obtain the pose of the drone's camera
    // frame.
    //
        ...
        if (adapter_mono.getNumberCorrespondences() >= min_nr_of_correspondences) {
          if (!use_ransac_) {
            ...
            opengv::essentials_t fivept_nister_essentials =
                opengv::relative_pose::fivept_nister( adapter_mono );
            
            int inliners = extractPose(fivept_nister_essentials , pts1 , pts2 ,camera_params_ , R , t);
            ...
            std::shared_ptr<RansacProblem>
            relposeproblem_ptr( new RansacProblem( adapter_mono, RansacProblem::NISTER ) );
        ...
        if (adapter_mono.getNumberCorrespondences() >= min_nr_of_correspondences) {
          if (!use_ransac_) {
            ...
            opengv::essential_t eightpt_essential =
                opengv::relative_pose::eightpt( adapter_mono );
            
            cv::Mat Emat;
            cv::eigen2cv(eightpt_essential, Emat);
            ...
            std::shared_ptr<RansacProblem>
            relposeproblem_ptr( new RansacProblem( adapter_mono, RansacProblem::EIGHTPT ) );
        ...
        if (adapter_mono.getNumberCorrespondences() >= min_nr_of_correspondences) {
          ...
          if (!use_ransac_) {
            ...
            opengv::translation_t twopt_translation =
                opengv::relative_pose::twopt( adapter_mono, true );
            // Eigen::Vector3d opengv::translation_t

            opengv::transformation_t full_transform;
            full_transform.block<3,3>(0,0) = adapter_mono.getR12(); // Use the known rotation
            full_transform.block<3,1>(0,3) = twopt_translation;
            relative_pose_estimate = eigen2Pose(full_transform);
            ...
            std::shared_ptr<RansacProblemGivenRot>
            relposeproblem_ptr( new RansacProblemGivenRot( adapter_mono ) );
      ...
      case 3: {
        ...
        if (adapter_3d.getNumberCorrespondences() >= min_nr_of_correspondences) {
          if (!use_ransac_) {
            // Without RANSAC (Optional)
            // ************************ begin solution  ************************
            opengv::transformation_t 	aruns_transformation = 
              opengv::point_cloud::threept_arun(adapter_3d) ; 

            relative_pose_estimate = eigen2Pose(aruns_transformation) ;
            // ************************ end solution **********************
          } else {
            // (TODO) With RANSAC
            // *********************** begin solution ***********************
            ...
            std::shared_ptr<RansacProblem3D> 
            poseproblem3d_ptr( new RansacProblem3D( adapter_3d ) );

            ransac.sac_model_ = poseproblem3d_ptr; 
            ransac.threshold_ = threshold; 
            ransac.max_iterations_ = maxIterations; 
            ransac.computeModel() ; 
