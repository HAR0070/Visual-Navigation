...
void createMeasurements(const Cal3_S2::shared_ptr& K,
  ...
  for (size_t i = 0; i < poses.size(); ++i) {
    PinholeCamera camera(poses[i], *K);
...
class SfmNode : public rclcpp::Node {
 public:
  rclcpp::Publisher<visualization_msgs::msg::Marker>::SharedPtr landmarks_gt_pub_;
  rclcpp::Publisher<visualization_msgs::msg::Marker>::SharedPtr landmarks_init_pub_;
  rclcpp::Publisher<visualization_msgs::msg::Marker>::SharedPtr landmarks_optimal_pub_;
  rclcpp::Publisher<geometry_msgs::msg::PoseArray>::SharedPtr poses_gt_pub_;
  rclcpp::Publisher<geometry_msgs::msg::PoseArray>::SharedPtr poses_init_pub_;
  rclcpp::Publisher<geometry_msgs::msg::PoseArray>::SharedPtr poses_optimal_pub_;

  SfmNode() : Node("sfm_node") {
    rclcpp::QoS latch = rclcpp::QoS(rclcpp::KeepLast(1)).transient_local();

    landmarks_gt_pub_ =
        create_publisher<visualization_msgs::msg::Marker>("/landmarks_gt", latch);
    landmarks_init_pub_ =
        create_publisher<visualization_msgs::msg::Marker>("/landmarks_init", latch);
    landmarks_optimal_pub_ =
        create_publisher<visualization_msgs::msg::Marker>("/landmarks_optimal", latch);
    poses_gt_pub_ = create_publisher<geometry_msgs::msg::PoseArray>("/poses_gt", latch);
    poses_init_pub_ =
        create_publisher<geometry_msgs::msg::PoseArray>("/poses_init", latch);
    poses_optimal_pub_ =
        create_publisher<geometry_msgs::msg::PoseArray>("/poses_optimal", latch);
    ...
    // TODO: Add a prior on pose x0. This indirectly specifies where the origin
    // is.
    // Insert code below:

    Pose3 prior_mean = camera_poses[0];  // come back 
    noiseModel::Diagonal::shared_ptr prior_noise = noiseModel::Diagonal::Sigmas(
        (Vector(6) << Vector3::Constant(0.1), Vector3::Constant(0.3)).finished());
        // Exercise mentions noise is our choice

    graph.add(PriorFactor<Pose3>(Symbol('x', 0), prior_mean, prior_noise)); 
    ...
    // component is the actual keypoint measurement in pixel coordinates
    // together with the idx of the corresponding landmark.
    std::vector<std::pair<size_t, Keypoint>> measurements;
    ...
    for( const auto& measure : measurements){
      gtsam::Key pose_key = gtsam::Symbol('x', measure.first);
      gtsam::Key point_key = gtsam::Symbol('l', measure.second.lmk_idx_); // landmark key
      GenericProjectionFactor<Pose3, Point3, Cal3_S2> factor(measure.second.kpt_coords_, measurementNoise , pose_key, point_key, K );
      graph.add(factor);
    }
    ...
    // Insert code below:
    Point3 landmark_prior_mean = landmarks[0];  // come back
    noiseModel::Isotropic::shared_ptr landmark_prior_noise =
        noiseModel::Isotropic::Sigma(3, 0.1);  // come back 
    graph.add(PriorFactor<Point3>(Symbol('l', 0), landmark_prior_mean, landmark_prior_noise));  
    ...
    DrawPoint3(landmarks_init_pub_, now(), initial_landmarks, marker_color::color_init);
    ...
    DrawPoses3(poses_init_pub_, now(), initial_poses);
    ...
    DrawPoint3(
        landmarks_optimal_pub_, now(), optimal_landmarks, marker_color::color_optimal);
    std::vector<Pose3> optimal_poses;
    for (int i = 0; i < camera_poses.size(); ++i) {
      optimal_poses.push_back(result.at<Pose3>(Symbol('x', i)));
    }
    DrawPoses3(poses_optimal_pub_, now(), optimal_poses);

    // Plot in Rviz the ground-truth of the above variables (camera poses
    // + landmarks).
    DrawPoint3(landmarks_gt_pub_, now(), landmarks, marker_color::color_gt);
    DrawPoses3(poses_gt_pub_, now(), camera_poses);
