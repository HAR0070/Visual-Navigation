 ...
 public:
  ...
  void run() {
    ...
    for (size_t i = 0; i < measurements.size(); ++i){
      graph.add(BetweenFactor<Pose3>(i+1 , i+2, measurements.at(i), odometryNoise));
    }
    ...
    if (!use_mocap_) {
      ...
      // Start of 2b.
      graph.add(PriorFactor<Pose3>(1, init_pose, initialNoise));
    ...
    if (use_mocap_) {
      ...
      noiseModel::Diagonal::shared_ptr mocap_noise_model =
      ...
      for (const auto& mocap_measurement : mocap_measurements) {
        // look at generateMoCapMeasurements for mocap_measurement format
        graph.add(std::make_shared<MoCapPosition3Factor>(mocap_measurement.first, mocap_measurement.second, mocap_noise_model));
      }
