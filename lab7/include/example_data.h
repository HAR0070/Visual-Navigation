...
std::vector<gtsam::Pose3> createPoses() {
  ...
  for(; i < 8; ++i, theta += 2*M_PI/8) {
    ...
    // given camera is at position, looking at target with up direction, compute the camera pose in the world frame
    gtsam::PinholeCamera<gtsam::Cal3_S2> camera = gtsam::PinholeCamera<gtsam::Cal3_S2>::Lookat(position, target, up);
    poses.push_back(camera.pose()); // this is camera pose in the world frame
  }
  return poses;
}
/* ************************************************************************* */
